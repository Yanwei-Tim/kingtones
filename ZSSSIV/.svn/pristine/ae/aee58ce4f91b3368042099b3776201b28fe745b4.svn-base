        ...
        while (i.hasNext()) {
            ...
            if (!operations.isEmpty()) {
                ...
                for (Iterator it = operations.iterator(); it.hasNext(); ) {
                    ...
                    List paraList = desc.getAllInParams();
                    
                    String paraNames = "";
                    for(int ii=0;ii<paraList.size();ii++){
                    	ParameterDesc pd = desc.getParameter(ii);
                    	paraNames+="split"+pd.getName();
                    }
                    //zoujiale：添加了 “测试” 超链接
                    String url = baseURL+sd.getName()+"?wsdl";
                    writer.println("<li>" + desc.getName()+"   <a href='axistest.jsp?para="+paraNames+"&method="+desc.getName()+"&url="+url+"'>测试</a>");
                }
                writer.println("</ul>");
            }
        }
