...
//----------------【定义flexgrid开始】--------------------------------------------------------------------------
	...
	var delVod ='none';
	 ...
	 for(var i=0;i<str.length;i++){
		 ...
		 if(temp.indexOf("ROLE_BUTTON")>=0){
			 temp1=temp.split("=")[1];
			 if(temp1.indexOf("downVod")>=0){
				 downVod='inline';
			 }
			 if(temp1.indexOf("delVod")>=0){
				 delVod='inline';
			 }
			 break;
		 }
	...
	$("#flex1").flexigrid(
	{
		url: webroot+'/commonbiz/sjcjAC!querySpcjList.action',
		dataType: 'json',
		colModel : [
			{display: '编号', name : 'id', width : 40, sortable : true, align: 'center',hide: false},
			{display: '视频名称', name : 'videoname', width : 180, sortable : true, align: 'left'},
		...
		buttons : [
			{name: '删除', onpress : btn_even,state:delVod},
			{separator: true,sepstate:delVod},
			{name: '下载视频', onpress : btn_even,state:downVod},
			{separator: true,sepstate:downVod}
			],
		sortname: "iso",
		sortorder: "asc",
		usepager: true,
	...
	function btn_even(com,grid){
		...
		else if (com=='删除')
		{
				if($('.trSelected',grid).length==0){
					...
					if(confirm('是否要删除该项！')){
						...
						$.ajax({
					    	url: webroot+'/commonbiz/sjcjAC!delSpcj.action?ids='+ids,
					    	type: 'POST',
						    dataType: 'json',
						    //timeout: 1000,
						    error: function(){
		...
		{   if($('.trSelected',grid).length==0){
					// 修改提示信息：“请选择要下载的项” 为 “请选择一项要下载的视频信息！” -Modify by 陈萧如
					alert("请选择一项需要要下载的视频信息！");
					return;
			}
			// 一次只能下载一条视频信息 - Add by 陈萧如 bug对应
			...
			$.ajax({
				url: webroot+'/commonbiz/sjcjAC!queryVedioUrlDir.action?id='+selectItemId,
				type: 'POST',
				dataType: 'json',
		       ...
		       window.location=webroot+'/commonbiz/sjcjAC!queryVedioUrl.action?id='+selectItemId;
		       }
				}
			});
