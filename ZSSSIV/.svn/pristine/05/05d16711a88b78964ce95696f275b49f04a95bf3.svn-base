...
*   程序名称：  Popform.js
*   程序功能：  实现日历功能
*   传入参数：  窗体标题（formtitle） 和窗体里显示的内容的html语句(formcontent)
*   作    者：  田彬
*   编号日期：  2007-7-1
*   调用示例：  <input type="button" onclick="new Calendar(0).show(this)">
*********************************************************************************************/
/************************************************
*   程序名称：  toDate
*   程序功能：  返回日期
*   传入参数：  分隔符（X）和日期格式(p)(可以为空值)
*   传出参数：  日期
*************************************************/
...
Calendar.prototype.draw = function() 
{
  ...
  var mvAry = [];
  mvAry[mvAry.length]  = '  <form name="calendarForm" style="width:175px"">';
  mvAry[mvAry.length]  = '    <table width="100%" border="0" cellpadding="0" cellspacing="0">';
  mvAry[mvAry.length]  = '      <tr>';
  mvAry[mvAry.length]  = '        <th style="text-align:left;width:1%"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:20px;" name="prevMonth" type="button" id="prevMonth" value="&lt;" /></th>';
  mvAry[mvAry.length]  = '        <th style="text-align:center;width:98%" nowrap="nowrap"><select name="calendarYear" id="calendarYear" style="font-size:12px;width:50%;"></select><select name="calendarMonth" id="calendarMonth" style="font-size:12px;width:50%;"></select></th>';
  mvAry[mvAry.length]  = '        <th style="text-align:right;width:1%"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:16px;height:20px;" name="nextMonth" type="button" id="nextMonth" value="&gt;" /></th>';
  mvAry[mvAry.length]  = '      </tr>';
  ...
  this.form.calendarToday.onclick = function () {
    ...
    //document.GetElementByID('隐藏控件的ID').click();
...
//显示日历
//显示日立方式，此日历为向上弹出
//可以调整this.panel.parentnode.left / top 的值来进行弹出调节
...
//    this.bindData();
  ...
  this.panel.parentNode.style.top = (xy.y - dateControl.offsetHeight-155) + "px";
...
var html = '<div style="\
	position:absolute;visibility:hidden;z-index:9999;background-color:#fff;border:2px solid #ccc;width:175px;font-size:12px;\
	"><div id="calendarPanel" height="180px"></div></div>';
document.write(html);
