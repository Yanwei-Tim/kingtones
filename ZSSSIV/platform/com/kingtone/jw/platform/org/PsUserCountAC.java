...
public class PsUserCountAC{
     ...
     * @throws Exception
     */
	public String findJqUserCount() throws Exception{
		rows.clear();
		JqCount jqcount = new JqCount();
		jqcount.setOrg_id(treeNodeID);
		List ulist = null;
		...
		ulist = psUCBS.findJqUserCount(jqcount,(Integer.parseInt(page)-1)*Integer.parseInt(rp),Integer.parseInt(rp));
        for(Object o :  ulist) {
        	JqCount jqcount1 = (JqCount) o;
			Map map = new HashMap();
			map.put("id", jqcount1.getId());
			map.put("cell", new Object[] { jqcount1.getId(),jqcount1.getName(),jqcount1.getAccount(),jqcount1.getOrg_name(),
					jqcount1.getAccept()==0?0:"<b><font color=red>"+jqcount1.getAccept()+"</font></b>",jqcount1.getReach()==0?0:"<b><font color=orange>"+jqcount1.getReach()+"</font></b>",
					    jqcount1.getFinish()==0?0:"<b><font color=blue>"+jqcount1.getFinish()+"</font></b>",formatter.format(jqcount1.getStime()),formatter.format(jqcount1.getEtime())});
			rows.add(map);
		}
        total = psUCBS.findJqUserCountye(jqcount)+"";
		treeNodeID = null;
		return "jsonResult";
     ...
     * @throws Exception
     */
    public String addJqDispatch() throws Exception{
    	/*
    	 * 关于Dispatch数据库表的status列的值范围为：0,1,2,  这三个值的意思如下：
    	 * 值为0：已受理，未到位。
    	 * 值为1：已到位，未完成。
    	 * 值为2：已完成。
    	 */  	 
    	dispatch_bak.setId(new UID().toString());//添加id
    	dispatch_bak.setUser_code("该列废用");//添加code,该列在数据库中是废列，古不使用。
    	
//    	Date date = new Date();
//    	dispatch_bak.setUser_id("0003");
//    	dispatch_bak.setDescription("这是测试值1");
//    	dispatch_bak.setStatus(2);
//    	dispatch_bak.setContact1("11111111111");
//    	dispatch_bak.setContact2("222222222");
//    	dispatch_bak.setAddress("中国西安");
//    	dispatch_bak.setGps_point("gps0001");
//    	dispatch_bak.setStart_time(date);
//    	dispatch_bak.setEnd_time(date);
    	
    	psUCBS.addDispatchJq(dispatch_bak);       
        return "jsonResult";//测试返回count_list.jsp
    }
     ...
     * @throws Exception
     */
    public String DispatchUpdate() throws Exception{
    	psUCBS.JqDispatchUpdate(dispatch_bak);
		return "jsonResult";
