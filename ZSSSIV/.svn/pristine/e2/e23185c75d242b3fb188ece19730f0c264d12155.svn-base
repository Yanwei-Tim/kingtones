//----------------【定义flexgrid开始】--------------------------------------------------------------------------
	$("#flex1").flexigrid(
	{
		...
		buttons : [
			{name: '显示',bclass:'edit', onpress : btn_even,state:'inline'},
			{separator: true,sepstate:'inline'},
			{name: '不显示', bclass:'delete', onpress : btn_even,state:'inline'},
			{separator: true,sepstate:'inline'}
			],
		searchitems : [
	...
	function btn_even(com,grid){
		...
		else if (com=='不显示')
		{
			if($('.trSelected',grid).length==0){
				...
				$.ajax({
			    	url: webroot+'/biz/serviceAC!updateBizResShow.action',
			    	data:'bizResIds='+bizResIds+'&bizRes.visible=1',
			    	type: 'POST',
				    dataType: 'json',
				    //timeout: 1000,
				    error: function(){
		...
		if($('.trSelected',grid).length==0){
				...
				$.ajax({
			    	url: webroot+'/biz/serviceAC!updateBizResShow.action',
			    	data:'bizResIds='+bizResIds+'&bizRes.visible=0',
			    	type: 'POST',
				    dataType: 'json',
				    //timeout: 1000,
				    error: function(){
