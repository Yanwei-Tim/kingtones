...
//----------------【定义flexgrid开始】--------------------------------------------------------------------------
	...
	$("#flex1").flexigrid(
	{
		url: webroot+'/jq/psAC!queryJqGridData.action',
		dataType: 'json',
		colModel : [
			{display: '机构编号', name : 'id', width : 40, sortable : true, align: 'center',hide: true},
			{display: '机构名称', name : 'name', width : 150, sortable : true, align: 'left'},							
			{display: '机构编码', name : 'code', width : 150, sortable : true, align: 'left'},
			{display: '父机构名称', name : 'parent_id', width : 180, sortable : true, align: 'left'},
			{display: '描述信息', name : 'description', width : 180, sortable : false, align: 'left'},
			{display: '排列顺序', name : 'sort', width : 80, sortable : false, align: 'left'}
			],
		buttons : [
		...
		searchitems : [
			...
			{display: '机构名称', name : 'name',isdefault: true},
			{display: '机构编码', name : 'code'},
			],
		sortname: "iso",//排序字段
		sortorder: "asc",//顺序
...
var selectItemId;
	function btn_even(com,grid){
		...
		{ 
		 ...
		 // window.open(url,window,'status:false;dialogWidth:450px;dialogHeight:400px;dialogLeft:300px;dialogTop:230px;center:yes');
          window.open(url,'opendialog','width=460,height=410,top=230,left=300');
			
		}	else if (com=='获取导入模板')
		{
		window.location=webroot+'/jq/psAC!ExcelTemp.action'
