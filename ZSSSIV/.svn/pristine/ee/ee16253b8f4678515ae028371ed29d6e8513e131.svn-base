...
//----------------【定义flexgrid开始】--------------------------------------------------------------------------
	...
	$("#flex1").flexigrid(
	{
		url: webroot+'/dw/jwUserDisplayAC!queryUserDispatch.action',
		dataType: 'json',
		colModel : [		           
			...
			{display: '警员调度状态', name : 'display', width : 160, sortable : false, align: 'center'}
			],
		buttons : [
		    {name: '<b><font color=green>点击修改警员调度状态</font></b>', onpress : btn_even,state:unlockjwuser},
	...
	function btn_even(com,grid){
		...
		else if (com=='<b><font color=green>点击修改警员调度状态</font></b>')
		{
			...
			if(selectItemId==''){
				if(confirm('确认批量设置？')){
					...
					url=webroot+"/biz/dw/jwdispatch_add.jsp";
