...
(function($){
			...
			$('thead tr:first th:visible',this.hDiv).each
				(
			 	function ()
					...
					//zoujiale : 添加checkbox修改
					//$('div:eq('+n+')',g.cDrag).css({'left':cdpos+'px'}).show();//zoujiale:注释掉
					//添加多选框
					if (p.checkbox) {
							 	...
							 	function ()
									{
										...
										td.align = this.align;
										//zoujiale：td.innerHTML = row.cell[idx]当列值为null时默认现实“”
			...
			populate: function () { //get latest data
				...
				if (p.params)
					...
					$.ajax({
					   ...
					   data: param,
					   dataType: p.dataType,
					   //zoujiale 为了解决翻页后 点击树节点 页面不重置问题
					   success: function(data){g.addData(data);p.newp = 0;},
						...
						function ()
							{
									...
									//$(this).empty().append(tdDiv).dblclick(function(e){alert(e.id);})
						...
						.hover(
							function(){
								...
								if (g.colCopy) 
									...
									var nw = jQuery(g.nBtn).width();//outerWidth()IE不支持 zoujiale
		...
		if (p.searchitems)
			{
				...
				$('.pDiv2',g.pDiv).prepend("<div class='btnseparator'></div>");//zoujiale 修改 去掉查询图标按钮
				...
				$(g.sDiv).append("<div class='sDiv2'>"+p.findtext+" <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"+sopt+"</select> <input type='button' value='查询' /></div>");

				//$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch();});
				//$('input[value=Clear]',g.sDiv).click(function(){$('input[name=q]',g.sDiv).val(''); p.query = ''; g.doSearch(); });
				$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch();});//p.query = '';});//zoujiale 修改 回车 提交 p.query = '';
				$('input[value=查询]',g.sDiv).click(function(){g.doSearch();});//p.query = ''; });//zoujiale 修改 点击按钮 提交p.query = '';
				$(g.bDiv).after(g.sDiv);	
				
				//zoujiale add 2011-05-11 直接显示search框
				if(p.test2){
	...
	$.fn.flexReload = function(p) { // function to reload grid
		...
		return this.each( function() {
				...
				$('input[name=q]').val('');//zoujiale:1291行衍生BUG点击树节点后 清空 查询条件
				this.p.query = '';//zoujiale:点击树节点后 清空 查询条件
				this.grid.populate();
				}
			});
	}; //end flexReload
