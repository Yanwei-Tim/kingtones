/*
功能描述：日期选择控件，可遮罩select控件
调用方法：<input type="text" name="sDate1" id="sDate1" value="" size="10" onClick="return Calendar('sDate1');" />
*/
...
'#Tools {width:204px;background:#71A0DC;padding:0px;}'+
...
function Calendar_Show(year,month,day)
{
 ...
 content+='<div id="Tools" style="border:1px solid #6A84C8;"><ul><li style="width:158px;"></li><li style="width:40px;"><a href="javascript:parent.Calendar_Clear();">&nbsp;清 空&nbsp;</a></li></ul></div>';
//层定位
 var Calendar=document.getElementById('divCalendar');
 
 
 Calendar.style.height=90+21*Trn+'px';  //框架高度
...
{ 
 ...
 Calendar_Show(parseInt(Sdate[0]),parseInt(readDate(Sdate[1])),parseInt(readDate(Sdate[2])));
}
function Calendar_GetDate(y,m,d)
{
if(!Calendar_obj2||d!=0){
 if(d>0){Calendar_obj.value=y+'-'+checkDate(m)+'-'+checkDate(d);}
 else{
  var objEvent=divCalendar.event||arguments.callee.caller.arguments[0];
  var srcElement=objEvent.srcElement;
  if(!srcElement){srcElement=objEvent.target;}
  Calendar_obj.value=y+'-'+checkDate(m)+'-'+checkDate(srcElement.innerHTML);
 }
}
else{
 if(IsLeapYear(y)&&m==2){num=MonthDNum[m]+1;}
 else{num=MonthDNum[m];}
 Calendar_obj.value=y+'-'+checkDate(m)+'01';
 Calendar_obj2.value=y+'-'+checkDate(m)+'-'+checkDate(num);
}
Calendar_hide();}
...
function checkDate(inputData)
{
    var temp=inputData.toString();
    if(temp.length==1)
    {
        return '0'+temp;
    }
